...
/*  Determine the number of keys in each bucket */
    ...
    for( i=0; i<NUM_KEYS; i++ ) {
        ...
        //__builtin_prefetch(&key_array[i+64]);
    	//__builtin_prefetch(&work_buff[key_array[i+32]>>shift],1);
...
/*  Sort into appropriate bucket */
    ...
    for( i=0; i<NUM_KEYS; i++ )  
    {
        ...
        //__builtin_prefetch(&key_array[i+64]);
    	//__builtin_prefetch(&bucket_ptrs[key_array[i+32]>>shift],1);
        //__builtin_prefetch(&key_buff2[bucket_ptrs[key_array[i+16<NUM_KEYS?i+16:i]>>shift]],1);
    }
...
/*  In this section, the keys themselves are used as their 
    ...
    individual population                                       */
        ...
        for ( k = m; k < bucket_ptrs[i]; k++ ) {
            //__builtin_prefetch(&key_buff_ptr2[k+64]);
    	    //__builtin_prefetch(&key_buff_ptr[key_buff_ptr2[k+32<NUM_KEYS?k+32:k]],1);
...
/*  In this section, the keys themselves are used as their 
    ...
    for( i=0; i<NUM_KEYS; i++ ) {
    	__builtin_prefetch(&key_buff_ptr2[i+64]);
    	if(i+32<NUM_KEYS)__builtin_prefetch(&work_buff[key_buff_ptr2[i+32]],1);
